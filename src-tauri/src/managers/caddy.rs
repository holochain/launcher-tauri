...
impl CaddyManager {
  pub async fn launch() -> Result<CaddyManager, String> {
    let caddy_port = portpicker::pick_unused_port().expect("No ports free");
    launch_caddy(caddy_port, admin_port, app_port, running_apps)?;

    Ok(CaddyManager { caddy_admin_port })
  }
...
{}",
  ...
  pub fn launch_caddy() -> Result<(), String> {
