...
///
/// **Important:** On macOS and Windows this is the same as profile_data_dir().s
...
/// At the time of writing, ${APP_NAME} = `holochain-launcher`
///
pub fn profile_logs_dir(profile: String) -> Result<PathBuf, String> {
  #[cfg(target_os = "macos")]
  let path = dirs_next::home_dir()
    .ok_or(String::from("Failed to get home dir"))?
    .join("Library/Logs")
    .join(APP_NAME)
    .join(profiles)
    .join(profile);
...
/// At the time of writing, ${APP_NAME} = `holochain-launcher`
///
fn profile_tauri_path(profile: String) -> Result<PathBuf, String> {
...
/// At the time of writing, ${APP_NAME} = `holochain-launcher`
///
pub fn holochain_version_data_dir(holochain_version: HolochainVersion, profile: String) -> Result<PathBuf, String> {
  let version: String = holochain_version.into();
  Ok(root_holochain_data_dir(profile)?.join(version))
}
