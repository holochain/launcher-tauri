    ...
    async checkForUiUpdates() {
      ...
      await Promise.allSettled(
        Object.values(updatableAppsByLocatorDna).map(async (apps) => {
          const entryHashes = apps.map((app) =>
            decodeHashFromBase64(
          ...
          try {
            console.log(
              "@checkForUiPudates: entryHashes: ",
              entryHashes.map((eh) => encodeHashToBase64(eh))
            );
            const happReleases: Array<HappReleaseEntry | undefined> =
              await getHappReleasesByEntryHashes(
    ...
    async updateGui() {
      ...
      try {
        ...
        if (appstoreAppInfo) {
          console.log(
            "@updateGui: ",
            encodeHashToBase64(this.selectedGuiUpdateLocator!.dna_hash)
          );
